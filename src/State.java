...
public class State 
{
	...
	public List<String> get_legal_moves(Environment env)
	{
		...
		{
		    case 0: // North
			if(location.y() == env.r || env.obstacles.contains(new Point2D(location.x(), location.y() + 1)))//If in northmost row, or obstacle 
			break;
			else
				moves.add("GO");
			break;
			...
			if(location.x() == env.c || env.obstacles.contains(new Point2D(location.x() + 1, location.y()))) //If in eastmost column, or obstacle in front.
			break;
			else
				moves.add("GO");
			break;
		    case 2: // South
			if(location.y() == 1 || env.obstacles.contains( new Point2D(location.x() , location.y() - 1 )))//If at bottom row, or obstacle in front.
			break;
			else
				moves.add("GO");
			break;
		    case 3: // West
			if(location.x() == 1 || env.obstacles.contains( new Point2D(location.x() - 1, location.y())))//If at westmost column, or obstacle in front.
			break;
			else
				moves.add("GO");
			break;
